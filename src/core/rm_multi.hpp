...
class rm_mixed_polarity_impl2 {
 public:
  ...
  rm_mixed_polarity_impl2(Ntk& ntk, rm_rewriting_params2 const& ps_ntk,
 ...
 private:
  ...
  uint32_t recursive_deref1(Ntk& ntk1, node<Ntk> const& n) {
    /* terminate? */
    if (ntk1.is_constant(n) || ntk1.is_pi(n)) return 0;

    /* recursively collect nodes */
    uint32_t value = NodeCostFn{}(ntk1, n);
    if (ntk1.is_and(n)) and_node_deref++;
  ...
  /**************************************************************************************************************/
  std::pair<int32_t, bool> recursive_ref_contains(Ntk& ntk1, node<Ntk> const& n,
                                                  node<Ntk> const& repl) {
    /* terminate? */
    if (ntk1.is_constant(n) || ntk1.is_pi(n)) return {0, false};
    ...
    ntk1.foreach_fanin(n, [&](auto const& s) {
      ...
      if (ntk1.incr_value(ntk1.get_node(s)) == 0) {
        const auto [v, c] =
            recursive_ref_contains(ntk1, ntk1.get_node(s), repl);
        value += v;
        contains = contains || c;
      }
    });
  ...
  /**************************************************************************************************************/
  void pw_opt_by_onset(
      std::map<int, std::map<int, int>> onset)  // on_set optimization
  {
    int x = 0;
    int i, j;
    ...
    int L = onset.size(),
        C = (onset[0]).size();  // L is the length and C is the width
    /*cout << L << " " << C << endl;*/
    if (L == 2) return;
    ...
    for (i = 0; i < C; i++) {
      for (j = 2; j < L; j++) {
        if (onset[j][i] != 0) break;
      }
      if (j == L) {
        C--;
        for (int k = i; k < C; k++) {  // Delete column i
          for (int m = 0; m < L; m++) {
    ...
    if (L == 3) {
      ...
      for (i = 0; i < C; i++) {
        ...
        if (i < C - 1) {
          if (expression_new.size() >= 2 &&
              expression_new[expression_new.size() - 2] == '!') {
    ...
    for (i = 2; i < L; i++) {
      for (j = 0; j < C; j++) {
        if (onset[i][j] == 1) x += pow(2, onset[1][j]);
      }
      if (x != 0) decimalism1.insert(x);
      if (x == 1)
    ...
    if (find(decimalism.begin(), decimalism.end(), 1) == decimalism.end() &&
        decimalism1.size() > 1) {
      int cout_row_0 = 0;
      int count_xor = 0;
      ...
      if (cout_row_0 % 2 != 0) {
        ...
        if (expression_new[expression_new.size() - 2] == '+')
          ...
          for (int jj = 0; jj < expression_new.size(); jj++) {
            if (jj != expression_new.size() - 2) str += expression_new[jj];
          }
          expression_new = str;
        }
      }
    ...
    int temp;
    for (i = 0; i < k; i++) {  // Swap CK to onset table behind
      for (j = 0; j < L; j++) {
        temp = onset[j][CK[i]];
    ...
    // Looking for LK
    vector<string> onset_string(
        L, "");  // Convert the onset table to a CK string stripping table
    int nn = 0;
    for (i = 0; i < L; i++) {
      for (j = 0; j < C - k; j++) {
    ...
    for (i = 2; i < onset_string.size(); i++) {
      int cnt =
          count(onset_string.begin() + 2, onset_string.end(), onset_string[i]);
    ...
    vector<string> onset_string1(
        L, "");  // Convert the onset table to the onset string table
    for (i = 0; i < L; i++) {
      for (j = 0; j < C; j++) {
    ...
    vector<string> onset_string_new;  // Stores the onset table after it has
                                      // been transformed
    ...
    if (flag1 == 1 && flag2 == 1 && onset_st1.size() > 2 &&
        onset_st2.size() > 2)
      expression_new += "[";
    if (flag1 == 1) pw_opt_by_onset(onset_st1);
    if (flag1 == 1 && flag2 == 1 && onset_st1.size() > 2 &&
        onset_st2.size() > 2)
      expression_new += "+";
    if (flag2 == 1) pw_opt_by_onset(onset_st2);
    if (flag1 == 1 && flag2 == 1 && onset_st1.size() > 2 &&
        onset_st2.size() > 2)
      expression_new += "]";

    if (flag_row_0 == 1) expression_new += "]";
  ...
  signal_t create_ntk_from_str(std::string const& s,
    ...
    if (s.size() == 0) inputs.push(ntk.get_constant(false));
  ...
  void search_for_optimal_polarity(vector<string>& minterm,
    ...
    for (int l = 0; l < polarity.size(); l++) {
      ...
      int count = 0;
      count = count_the_number_of_nodes(variate_num, RM_product_initial,
                                        polarity[l]);
  ...
  void ntk_cut() {
    ...
    progress_bar pbar{ntk.size(),
                      "rm_optimization_by_cut |{0}| node = {1:>4}@{2:>2} / " +
    ...
    ntk.foreach_node([&](auto const& n, auto index) {
      ...
      for (auto& cut : cuts.cuts(ntk.node_to_index(n))) {
        ...
        int32_t value = recursive_deref1(ntk, n);
        {
          ...
          /* Search for the optimal polarity and the corresponding product
           * term.*/
          search_for_optimal_polarity(minterm, polarity, variate_num,
                                      RM_product, optimal_polarity);

          int pos = optimal_polarity.find("2");
          string expression = "";
          if (pos != optimal_polarity.npos) {
            expression = create_expression(RM_product, variate_num,
          ...
          and_node_ref = 0;
          auto [v, contains] =
              recursive_ref_contains(ntk, ntk.get_node(f_new), n);
    ...
    for (const auto v : is) {
      ...
      if (best_replacements[v_node].empty()) continue;
  ...
  void ntk_mffc() {
    progress_bar pbar{ntk.size(),
                      "rm_optimization_by_mffc |{0}| node = {1:>4}   cand = "
                      "{2:>4}   est. reduction = {3:>5}",
                      ps.progress};
    ...
    ntk.foreach_gate([&](auto const& n, auto i) {
      ...
      if (pos != optimal_polarity.npos) {
        ...
        for (int i = 0; i < variate_num; i++) onset[1][i] = variate_num - i - 1;
      ...
      if (ps_ntk.multiplicative_complexity == true) {
        if ((gain > 0 && and_node_num >= 0)) {
          ...
          ntk.set_value(n, 0);
          ntk.set_value(ntk.get_node(f_new),
                        ntk.fanout_size(ntk.get_node(f_new)));
          for (auto i = 0u; i < leaves.size(); i++) {
            ntk.set_value(ntk.get_node(leaves[i]),
                          ntk.fanout_size(ntk.get_node(leaves[i])));
          }
        } else {
...
void rm_mixed_polarity2(Ntk& ntk, rm_rewriting_params2 const& ps_ntk = {},
                        cut_rewriting_params const& ps = {}) {
