...
struct lut_map_stats {
  ...
  void report() const {
    ...
    std::cout << fmt::format("[CPU time]   = {:>5.2f} s\n",
...
enum class lut_cut_sort_type { DELAY, DELAY2, AREA, NONE };
...
class lut_cut_set {
 public:
  ...
  static bool sort_delay(CutType const& c1, CutType const& c2) {
    ...
    if (c1->data.area_flow < c2->data.area_flow - eps) return true;
    if (c1->data.area_flow > c2->data.area_flow + eps) return false;
  ...
  static bool sort_delay2(CutType const& c1, CutType const& c2) {
    ...
    if (c1->data.area_flow < c2->data.area_flow - eps) return true;
    if (c1->data.area_flow > c2->data.area_flow + eps) return false;
    if (c1->data.edge_flow < c2->data.edge_flow - eps) return true;
    if (c1->data.edge_flow > c2->data.edge_flow + eps) return false;
  ...
  void simple_insert(CutType const& cut,
    ...
    if (sort == lut_cut_sort_type::DELAY) {
      ...
      ipos = std::lower_bound(_pcuts.begin(), _pend, &cut,
                              [](auto a, auto b) { return sort_area(*a, *b); });
    } else /* NONE */
    {
      ipos = _pend;
    }
  ...
  void insert(CutType const& cut, bool skip0 = false,
    ...
    if (skip0 && _pend != _pcuts.begin()) ++begin;

    /* remove elements that are dominated by new cut */
    _pcend = _pend = std::stable_partition(
        begin, _pend,
        [&cut](auto const* other) { return !cut.dominates(*other); });
...
class lut_map_impl {
 ...
 public:
  ...
  void print() {
    ...
    stats << fmt::format("Delay = {:5d}  Area = {:6d}  Edges = {:7d}\n", delay,
                         area, edges);
 ...
 private:
  ...
  void init_cuts() {
    ...
    /* init PIs cuts */
    ntk.foreach_pi([&](auto const& n) { add_unit_cut(ntk.node_to_index(n)); });
  }
      ...
      if constexpr (!ELA) {
        ...
        if (!preprocess && iteration != 0) {
          node_match[index].est_refs =
              (2.0 * node_match[index].est_refs + node_match[index].map_refs) /
              3.0;
        } else {
          node_match[index].est_refs =
  ...
  void expand_cuts() {
    ...
    if constexpr (StoreFunction) return;

    /* don't expand if cut recomputed cuts is off */
    if (!ps.recompute_cuts) return;
  ...
  void set_mapping_refs() {
    ...
    for (auto it = top_order.rbegin(); it != top_order.rend(); ++it) {
      ...
      if (node_match[index].map_refs == 0u) continue;
  ...
  void compute_required_time() {
    ...
    if (iteration == 0 || ps.area_oriented_mapping) return;
    ...
    for (auto it = top_order.rbegin(); it != top_order.rend(); ++it) {
      if (ntk.is_pi(*it) || ntk.is_constant(*it)) continue;

      const auto index = ntk.node_to_index(*it);

      if (node_match[index].map_refs == 0) continue;

      for (auto leaf : cuts[index][0]) {
        node_match[leaf].required =
            std::min(node_match[leaf].required, node_match[index].required - 1);
      }
    }
  }
  ...
  void expand_cuts_node(node const& n) {
    ...
    if (node_data.map_refs == 0) return;
    ...
    for (auto const leaf : best_cut) {
      ...
      /* MFFC leaves */
      if (node_match[leaf].map_refs == 0) ++cost_before;
    }
    ...
    for (auto const leaf : leaves) {
      /* MFFC leaves */
      if (node_match[leaf].map_refs == 0) ++cost_after;
    }
  ...
  bool improve_cut(std::vector<uint32_t>& leaves) {
    if (improve_cut_expand0(leaves)) return true;
  ...
  bool improve_cut_expand0(std::vector<uint32_t>& leaves) {
    for (auto it = leaves.begin(); it != leaves.end(); ++it) {
      ...
      marked = 0;
      if (node_match[*it].map_refs == 0) --marked;
  ...
  void mark_cut_volume_rec(node const& n) {
    if (ntk.visited(n) == ntk.trav_id()) return;

    ntk.set_visited(n, ntk.trav_id());

    ntk.foreach_fanin(
        n, [&](auto const& f) { mark_cut_volume_rec(ntk.get_node(f)); });
  }
...
void lut_map(Ntk& ntk, lut_map_params const& ps = {},
             lut_map_stats* pst = nullptr) {
