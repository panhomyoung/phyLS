...
struct result_klut {
  ...
  vector<klut> possible_result;
};

class exact_lut_impl {
 public:
  exact_lut_impl(vector<string>& tt, int& input, int& cut_size)
      : tt(tt), input(input), cut_size(cut_size) {}

  void run() { exact_lut_network(); }
  void run_enu() { exact_lut_network_enu(); }
  void run_lut() { exact_lut_mapping(); }
  void run_lut_mapping() { exactLutMapping(); }
  vector<vector<klut>> exact_lut_results;
  vector<klut> exact_lut_result;
 ...
 private:
  ...
  void exact_lut_mapping() {
    int num_node = compute_nr_node();
    int num_level = 2;
    stopwatch<>::duration time{0};
    while (1) {
      // create all possible k-LUT DAGs
      vector<vector<klut>> lut;
      call_with_stopwatch(time, [&]() {
        create_kluts(lut, num_node, num_level);
        sort_luts(lut);
      });
      ...
      if (lut.size()) {
        ...
        for (int i = 0; i < lut.size(); i++) {
          ...
          for (int j = 0; j < lut[i].size(); j++) {
            result += to_string(lut[i][j].node);
            result += " = ";
            int num_input = pow(2, lut[i][j].inputs.size());
            result += to_string(num_input);
            result += "'b";
            result += lut[i][j].tt;
            result += " (";
            for (int k = 0; k < lut[i][j].inputs.size(); k++) {
              if (lut[i][j].inputs[k] <= input) {
                char temp;
                temp = 'a' + lut[i][j].inputs[k] - 1;
                result.push_back(temp);
              } else {
                result += to_string(lut[i][j].inputs[k]);
              }
              if (k != lut[i][j].inputs.size() - 1) result += ", ";
            }
            result += ")  ";
          }
          result_final.push_back(result);
    ...
    std::cout << fmt::format("[LUT generation time]: {:5.3f} seconds\n",
                             to_seconds(time));
  }

  void exactLutMapping() {
    int num_node = compute_nr_node();
    int num_level = 2;
    while (1) {
      // create all possible k-LUT DAGs
      ...
      if (lut.size()) {
        ...
        exact_lut_result.assign(lut[0].begin(), lut[0].end());
        break;
      } else {
        if (num_level < num_node) {
          num_level += 1;
  ...
  int compute_nr_node() {
    int num_node = 1;
    while (1) {
      int empty_node = ((cut_size - 1) * num_node) + 1;
      if (empty_node < input)
        num_node += 1;
      else
        break;
  ...
  void create_dags(vector<vector<phyLS::bench>>& lut_dags, int node,
                   int level) {
    phyLS::dag lut_dag_init;
    ...
    if (numlevels > 0) {
      if (numnodes == 0) {
        vector<vector<phyLS::bench>> lut_dags_temp;
        ...
        for (int i = 0; i < q.size(); i++) {
          phyLS::dag lut_dag_temp = lut_dag_init;
  ...
  void create_kluts(vector<vector<klut>>& lut_dags, int node, int level) {
    phyLS::dag lut_dag_init;
    lut_dag_init.node = node;
    lut_dag_init.level = level;
    lut_dag_init.count.resize(level + 1, 1);
    ...
    if (level > 1) {
      if (numnodes == 0) {
        klut_dag(lut_dag_init, lut_dags);
      } else {
        // enumerate the placement of all possible additional nodes
        vector<vector<int>> q;
        dag_classify(q, numnodes, level - 1);
        // enumerate all situations
        for (int i = 0; i < q.size(); i++) {
          phyLS::dag lut_dag_temp = lut_dag_init;
          for (int j = 1, k = 0; j < level; j++, k++)
            lut_dag_temp.count[j] += q[i][k];
          // judge whether lut is legitimate
          judge_lut(lut_dag_temp);
          if (!lut_dag_temp.count.empty()) klut_dag(lut_dag_temp, lut_dags);
        }
      }
    } else {
      // TODO: only one LUT, generate directly
    }
  }
  ...
  void judge_dag(phyLS::dag& lut_dag) {
  ...
  void judge_lut(phyLS::dag& lut_dag) {
    for (int i = 0; i < lut_dag.count.size() - 1; i++) {
      int sum =
          accumulate(lut_dag.count.begin() + i + 1, lut_dag.count.end() - 1, 0);
      int nr_possible = cut_size * (sum + 1) - sum;
      if (lut_dag.count[i] > nr_possible) {
        lut_dag.count.clear();
        break;
      }
    }
  ...
  void bench_dag(phyLS::dag lut_dag, vector<vector<phyLS::bench>>& lut_dags) {
    ...
    for (int i = 0; i < lut_dag.count.size(); i++, level_temp++) {
      vector<phyLS::node_inf> node_vec;
      sum += lut_dag.count[i];
    ...
    */
    ...
    for (int i = lut_dag.lut.size() - 1; i >= 0; i--) {
      vector<phyLS::bench> lut_dags_mid;
      ...
      for (int j = lut_dag.lut[i].size() - 1; j >= 0; j--) {
        if (lut_dag.lut[i][j].fan_out) {
          phyLS::bench LUT_fanout;
          LUT_fanout.node = lut_dag.lut[i][j].num;
          vector<phyLS::bench> lut_dags_fanout;
      ...
      // 仅针对中间节点进行排列组合
      if (permutation.size()) {
        vector<vector<phyLS::bench>> lut_dags_result;
        // 第2.2个for循环
        // 遍历已存在的所有结果(lut_dags)
        for (int k = 0; k < lut_dags.size(); k++) {
          vector<phyLS::bench> lut_dags_rst_temp;  // 尚未满足的节点
          vector<phyLS::bench> lut_dags_rst;       // 已满足的节点
          vector<int> count;
          // 第2.2.1个for循环
          // 为排列组合做准备
          // 判断还有哪些节点未填满，并确定剩余空间
          ...
          if (result.size()) {
            if (i == 0) {
              for (int m = 0; m < result.size(); m++) {
                bool target = 1;
                vector<phyLS::bench> result_temp1;
                int n2 = 0;
                for (int n1 = 0; n1 < lut_dags_rst_temp.size(); n1++) {
                  phyLS::bench result_temp_temp = lut_dags_rst_temp[n1];
  ...
  void klut_dag(phyLS::dag lut_dag, vector<vector<klut>>& lut_dags) {
    // 第一个for循环
    // 通过lut_dag的count，初始化lut_dag的lut
    // 编号每一个节点，并且将其放置在对应的层中
    // 每一层就是lut的层
    ...
    for (int i = lut_dag.lut.size() - 1; i >= 0; i--) {
      ...
      // 仅针对中间节点进行排列组合
      if (permutation.size()) {
        vector<vector<klut>> lut_dags_result;
        // 第2.2个for循环
        // 遍历已存在的所有结果(lut_dags)
        for (int k = 0; k < lut_dags.size(); k++) {
          vector<klut> lut_dags_rst_temp;  // 尚未满足的节点
          vector<klut> lut_dags_rst;       // 已满足的节点
          vector<int> count;
          // 第2.2.1个for循环
          // 为排列组合做准备
          // 判断还有哪些节点未填满，并确定剩余空间
          for (int l = 0; l < lut_dags[k].size(); l++) {
            if (lut_dags[k][l].inputs.size() < cut_size) {
              count.push_back(cut_size - lut_dags[k][l].inputs.size());
              lut_dags_rst_temp.push_back(lut_dags[k][l]);
            } else {
              lut_dags_rst.push_back(lut_dags[k][l]);
            }
          ...
          // result有结果
          if (result.size()) {
            if (i == 0) {  // primary inputs
              for (int m = 0; m < result.size(); m++) {
                bool target = true;
                vector<klut> result_temp1;
                for (int n1 = 0; n1 < lut_dags_rst_temp.size(); n1++) {
                  klut result_temp_temp = lut_dags_rst_temp[n1];
                  int nr_input_result =
                      nr_result_nodes(result_temp_temp.inputs);
                  while (nr_input_result < cut_size) {
                    if (result[m][0] != 0)
                      result_temp_temp.inputs.push_back(result[m][0]);
                    nr_input_result++;
                    result[m].erase(result[m].begin());
                  }
                  if (nr_result_nodes(result_temp_temp.inputs) < 2)
                    target = false;
                  if (!target) break;
                  sort(result_temp_temp.inputs.begin(),
                       result_temp_temp.inputs.end());
                  result_temp1.push_back(result_temp_temp);
                }
                if (target) {
                  if (lut_dags_mid.size()) {
  ...
  int nr_result_nodes(vector<int>& result) {
    int nr_result_node = 0;
    for (auto& x : result) {
      if (x != 0) nr_result_node += 1;
    }
    return nr_result_node;
  }

  void sort_luts(vector<vector<klut>>& luts) {
    for (int i = 0; i < luts.size(); i++) {
      Quicksort_lut(luts[i], 0, luts[i].size() - 1);
    }
  }

  void Quicksort_lut(vector<klut>& lut, int low, int hight) {
    int mid;
    if (low < hight) {
      mid = part_lut(lut, low, hight);     // 返回基准元素位置
      Quicksort_lut(lut, low, mid - 1);    // 左区间递归快速排序
      Quicksort_lut(lut, mid + 1, hight);  // 右区间递归快速排序
    }
  }

  int part_lut(vector<klut>& lut, int low, int hight)  // 划分函数
  {
    int i = low, j = hight, pivot = lut[low].node;  // 基准元素
    while (i < j) {
      while (i < j &&
             lut[j].node < pivot)  // 从右向左开始找一个 小于等于 pivot的数值
      {
        j--;
      }
      if (i < j) {
        swap(lut[i++], lut[j]);  // r[i]和r[j]交换后 i 向右移动一位
      }
      while (i < j &&
             lut[i].node >= pivot)  // 从左向右开始找一个 大于 pivot的数值
      {
        i++;
      }
      if (i < j) {
        swap(lut[i], lut[j--]);  // r[i]和r[j]交换后 i 向左移动一位
      }
    }
    return i;  // 返回最终划分完成后基准元素所在的位置
  }

  void sort_dags(vector<vector<phyLS::bench>>& luts) {
    for (int i = 0; i < luts.size(); i++) {
      Quicksort(luts[i], 0, luts[i].size() - 1);
    }
  }

  void Quicksort(vector<phyLS::bench>& lut, int low, int hight) {
    int mid;
    if (low < hight) {
      mid = part(lut, low, hight);     // 返回基准元素位置
      Quicksort(lut, low, mid - 1);    // 左区间递归快速排序
      Quicksort(lut, mid + 1, hight);  // 右区间递归快速排序
    }
  }

  int part(vector<phyLS::bench>& lut, int low, int hight)  // 划分函数
  {
    int i = low, j = hight, pivot = lut[low].node;  // 基准元素
    while (i < j) {
      while (i < j &&
  ...
  void stp_simulate(vector<vector<phyLS::bench>>& lut_dags) {
    string tt_binary = tt[0];
    int flag_node = lut_dags[0][lut_dags[0].size() - 1].node;
    vector<vector<phyLS::bench>> lut_dags_new;
    for (int i = lut_dags.size() - 1; i >= 0; i--) {
      string input_tt(tt_binary);

      vector<phyLS::matrix> matrix_form =
          chain_to_matrix(lut_dags[i], flag_node);
      matrix_computution(matrix_form);

      for (auto y : lut_dags[i]) {
        cout << y.node << "-(" << y.left << ", " << y.right << ")  ";
      }
      cout << "=  ";
      for (int i = 0; i < matrix_form.size(); i++) {
        cout << matrix_form[i].name << "_" << matrix_form[i].node << "[I"
             << matrix_form[i].eye << "]  ";
      }
      cout << endl;

      vector<phyLS::result_lut> bench_result;
      phyLS::result_lut first_result;
      ...
      for (int l = matrix_form.size() - 1; l >= 1; l--) {
        ...
        if (matrix_form[l].input == 0) {
          if (matrix_form[l].name == "W") {
            ...
            vector<phyLS::result_lut> bench_result_temp;
            for (int q = bench_result.size() - 1; q >= 0; q--) {
              ...
              if (result.empty()) {
                ...
                for (int m = 0; m < result.size(); m++) {
                  ...
                  phyLS::result_lut result_temp_1, result_temp_2;
      ...
      for (int j = bench_result.size() - 1; j >= 0; j--) {
        ...
        vector<phyLS::result_lut> bench_temp;
        for (int k = in_expansion.size() - 1; k >= 0; k--) {
          ...
          if (!compare_result(in_expansion[k], tt_binary)) {
            ...
            phyLS::result_lut bench_temp_temp = bench_result[j];
  ...
  void stp_simulate_enu(vector<vector<phyLS::bench>>& lut_dags) {
    string tt_binary = tt[0];
    int flag_node = lut_dags[0][lut_dags[0].size() - 1].node;
    vector<vector<phyLS::bench>> lut_dags_new;
    for (int i = lut_dags.size() - 1; i >= 0; i--) {
  ...
  void stp_simulate_klut(vector<vector<klut>>& lut_dags) {
    string tt_binary = tt[0];
    int flag_node = lut_dags[0][lut_dags[0].size() - 1].node;
    vector<vector<klut>> lut_dags_new;
    for (int i = lut_dags.size() - 1; i >= 0; i--) {
      string input_tt(tt_binary);  // input function
      vector<matrix_klut> matrix_form = klut_to_matrix(
          lut_dags[i], flag_node);  // STP-based phyLS::matrix form
      matrix_computution_klut(matrix_form);
      ...
      // cout << endl;
      // for (int i = 0; i < matrix_form.size(); i++) {
      //   cout << matrix_form[i].name << "_" << matrix_form[i].node << "("
      //        << matrix_form[i].nr_input << ")[I" << matrix_form[i].eye <<
      //        "]  ";
      // }
      // cout << endl;

      vector<result_klut> bench_result;
      result_klut first_result;
      first_result.computed_input = input_tt;
      first_result.possible_result = lut_dags[i];
      bench_result.push_back(first_result);
      for (int l = matrix_form.size() - 1; l >= 1; l--) {
        if (!matrix_form[l].input) {
          int length_string = input_tt.size();
          if (matrix_form[l].name == "W") {
            if (matrix_form[l].eye == 0) {
              string temp1, temp2;
            ...
            vector<result_klut> bench_result_temp;
            for (int q = bench_result.size() - 1; q >= 0; q--) {
              int length_string2 = bench_result[q].computed_input.size();
              //   cout << "computed input: " << bench_result[q].computed_input
              //        << endl;
              int length1 =
                  length_string2 /
                  pow(2.0, matrix_form[l].eye +
                               matrix_form[l].nr_input);  // abcd的长度
              ...
              for (int l3 = 0; l3 < pow(2.0, matrix_form[l].eye); l3++) {
                ...
                for (int l4 = 0; l4 < pow(2.0, matrix_form[l].nr_input); l4++) {
                  string a = bench_result[q].computed_input.substr(
                      ind + (length1 * l4), length1);
                  input_parts.push_back(a);
                }
                // compare all input_parts have only two modes
                bool enableFactor = true;
                string pattern1, pattern2;
                vector<int> placement;
                for (auto x : input_parts) {
                  if (pattern1.empty()) {
                    pattern1 = x;
                    placement.push_back(0);
                  } else if (!matchPattern(x, pattern1)) {
                    if (pattern2.empty()) {
                      pattern2 = x;
                      placement.push_back(1);
                    } else if (!matchPattern(x, pattern2)) {
                      enableFactor = false;
                      break;
                    } else {
                      replaceTwos(x, pattern2);
                      placement.push_back(1);
                    }
                  } else {
                    replaceTwos(x, pattern1);
                    placement.push_back(0);
                  }
                }
                if (enableFactor) {
                  //   cout << "pattern 1/2: " << pattern1 << "/" << pattern2
                  //        << endl;
                  bool enableFactor2 = true;
                  if (patterns.empty()) {
                    patterns.push_back(pattern1);
                    patterns.push_back(pattern2);
                    placements.assign(placement.begin(), placement.end());
                  } else {
                    enableFactor2 = compare_vector(placement, placements);
                    if (enableFactor2) {
                      patterns.push_back(pattern1);
                      patterns.push_back(pattern2);
                    } else {
              ...
              if (patterns.empty()) {
                ...
                // }
                // cout << endl;
                string count1, count2, res1, res2;
                for (int m = 0; m < placements.size(); m++) {
                  count1 += to_string(placements[m]);
                  count2 += to_string(1 - placements[m]);
                }
                // cout << "count 1/2: " << count1 << "/" << count2 << endl;
                int tt_length = pow(2.0, matrix_form[l].nr_input);
                string wrong_result1(tt_length, '1');
                string wrong_result2(tt_length, '0');
                if (count1 == wrong_result1 || count1 == wrong_result2 ||
                    count2 == wrong_result1 || count2 == wrong_result2) {
                  bench_result.erase(bench_result.begin() + q);
                  continue;
                }
                for (int n = 0, p = 0; n < pow(2.0, matrix_form[l].eye); n++) {
                  if (placements[0] == 1) {
                    res1 += patterns[p];
                    res1 += patterns[p + 1];
                    res2 += patterns[p + 1];
                ...
                // cout << "res 1/2: " << res1 << "/" << res2 << endl;
                result_klut result_t1, result_t2;
                result_t1.possible_result = bench_result[q].possible_result;
                result_t2.possible_result = bench_result[q].possible_result;
                for (int p = 0; p < result_t1.possible_result.size(); p++) {
                  if (result_t1.possible_result[p].node == matrix_form[l].node)
                    result_t1.possible_result[p].tt = count1;
                  if (result_t2.possible_result[p].node == matrix_form[l].node)
                    result_t2.possible_result[p].tt = count2;
                }
                if (res1.size() == pow(2, matrix_form[0].nr_input)) {
                  for (int p = 0; p < result_t1.possible_result.size(); p++) {
                    if (result_t1.possible_result[p].node ==
                        matrix_form[0].node)
                      result_t1.possible_result[p].tt = res1;
                    if (result_t2.possible_result[p].node ==
                        matrix_form[0].node)
                      result_t2.possible_result[p].tt = res2;
                  }
                } else {
                  result_t1.computed_input = res1;
                  result_t2.computed_input = res2;
                }
                bench_result_temp.push_back(result_t1);
                bench_result_temp.push_back(result_t2);
              }
            }
            if (bench_result_temp.empty())
              break;
  ...
  vector<phyLS::matrix> chain_to_matrix(vector<phyLS::bench> lut_dags,
                                        int flag_node) {
    vector<phyLS::matrix> matrix_form;
    phyLS::matrix temp_node, temp_left, temp_right;
    ...
    if (lut_dags[0].right >= flag_node) {
      vector<phyLS::matrix> matrix_form_temp2;
    ...
    if (lut_dags[0].left >= flag_node) {
      vector<phyLS::matrix> matrix_form_temp1;
  ...
  vector<matrix_klut> klut_to_matrix(vector<klut> lut_dags, int flag_node) {
    vector<matrix_klut> matrix_form;
    matrix_klut temp_node;
    temp_node.node = lut_dags[0].node;
    temp_node.name = "M";
    temp_node.nr_input = lut_dags[0].inputs.size();
    matrix_form.push_back(temp_node);
    for (int i = lut_dags[0].inputs.size() - 1; i >= 0; i--) {
      if (lut_dags[0].inputs[i] >= flag_node) {
        vector<matrix_klut> matrix_form_temp2;
        matrix_generate_klut(lut_dags, matrix_form_temp2,
                             lut_dags[0].inputs[i]);
        matrix_form.insert(matrix_form.end(), matrix_form_temp2.begin(),
                           matrix_form_temp2.end());
      } else {
        matrix_klut temp_node_input;
        temp_node_input.node = lut_dags[0].inputs[i];
        temp_node_input.input = true;
        string str = to_string(lut_dags[0].inputs[i]);
        temp_node_input.name = str;
        matrix_form.push_back(temp_node_input);
      }
    }
    return matrix_form;
  }

  void matrix_computution(vector<phyLS::matrix>& matrix_form) {
    bool status = true;
    while (status) {
      status = false;
  ...
  void matrix_computution_klut(vector<matrix_klut>& matrix_form) {
    bool status = true;
    while (status) {
      status = false;
      for (int i = matrix_form.size() - 1; i > 1; i--) {
        ...
        if (((matrix_form[i].name != "M") && (matrix_form[i].name != "R") &&
          ...
          if (left == right) {
            status = true;
            matrix_klut reduce_matrix;
            reduce_matrix.name = "R";
            matrix_form.insert(matrix_form.begin() + (i - 1), reduce_matrix);
            matrix_form.erase(matrix_form.begin() + i);
          } else if (left < right) {
            status = true;
            matrix_klut swap_matrix;
            swap_matrix.name = "W";
            swap(matrix_form[i - 1], matrix_form[i]);
            matrix_form.insert(matrix_form.begin() + (i - 1), swap_matrix);
          }
  ...
  void matrix_generate(vector<phyLS::bench> lut,
                       vector<phyLS::matrix>& matrix_form, int node) {
  ...
  void matrix_generate_klut(vector<klut> lut, vector<matrix_klut>& matrix_form,
                            int node) {
    int flag = lut[lut.size() - 1].node;
    for (int i = 0; i < lut.size(); i++) {
      if (lut[i].node == node) {
        ...
        temp_node.node = node;
        temp_node.name = "M";
        temp_node.nr_input = lut[i].inputs.size();
        matrix_form.push_back(temp_node);
        for (int j = lut[i].inputs.size() - 1; j >= 0; j--) {
          if (lut[i].inputs[j] >= flag) {
            vector<matrix_klut> matrix_form_temp2;
            matrix_generate_klut(lut, matrix_form_temp2, lut[i].inputs[j]);
            matrix_form.insert(matrix_form.end(), matrix_form_temp2.begin(),
                               matrix_form_temp2.end());
          } else {
            matrix_klut temp_node_input;
            temp_node_input.node = lut[i].inputs[j];
            temp_node_input.input = 1;
            string str = to_string(lut[i].inputs[j]);
            temp_node_input.name = str;
            matrix_form.push_back(temp_node_input);
          }
        }
  ...
  bool matchPattern(string str, string pattern) {
    for (size_t i = 0; i < pattern.length(); i++) {
      if (pattern[i] == '2' || str[i] == '2') continue;
      if (pattern[i] != str[i]) return false;
    }
    return true;
  }

  void replaceTwos(string str, string& pattern) {
    for (size_t i = 0; i < pattern.length(); i++) {
      if (pattern[i] == '2') pattern[i] = str[i];
    }
  }
  ...
  void bench_solve(vector<string>& in, vector<vector<int>> mtxvec) {
    ...
    vector<vector<phyLS::cdccl_impl>>
        list;  // the solution space is two dimension vector
    vector<phyLS::cdccl_impl> list_temp;
    phyLS::cdccl_impl list_temp_temp;

    string Result_temp(length1, '2');  // temporary result
    phyLS::coordinate Level_temp;
    ...
    for (int level = 0;; level++)  // the computation process
    {
      ...
      vector<phyLS::cdccl_impl> list_temp1;
      for (int k = 0; k < list[level].size(); k++) {
        phyLS::cdccl_impl temp1;               // temporary gate
        ...
        for (int j = 0; j < list[level][k].Intermediate.size(); j++) {
          ...
          phyLS::coordinate level_current;  // new level
...
void exact_lut(vector<string>& tt, int& input, int& cut_size) {
  exact_lut_impl p(tt, input, cut_size);
  p.run();
}

void exact_lut_enu(vector<string>& tt, int& input, int& cut_size) {
  exact_lut_impl p(tt, input, cut_size);
  p.run_enu();
}

void exact_lut_mapping(vector<string>& tt, int& input, int& cut_size) {
  exact_lut_impl p(tt, input, cut_size);
  p.run_lut();
}
}  // namespace phyLS
